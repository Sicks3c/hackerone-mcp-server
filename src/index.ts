...
server.tool(
  "update_report_draft",
  "Replace a HAI report draft (report intent) with a new one carrying the new description: fetches the old draft and creates a new draft for the same program, returning immediately with the new draft ID (HAI jobs run asynchronously — poll the new draft with get_report_draft until state is 'ready_to_submit'). The old draft is KEPT; delete it with delete_report_draft once the new one is ready. The title is re-generated by HAI. Attachments are NOT carried over — re-upload them to the new draft and update any {F<id>} references in the description.",
  {
...
server.tool(
  "delete_report_draft",
  "Delete a HAI report draft (report intent) by ID. Irreversible; only possible while the draft has not been submitted. Typically used to remove the old draft after update_report_draft created a replacement.",
  {
